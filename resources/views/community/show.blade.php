...
<div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-12" x-data="postPage()">
    ...
    <div class="bg-white rounded-2xl border border-gray-100 p-8">
        <div class="flex items-start space-x-4 mb-6">
            <img src="{{ $post->user->avatar ?? 'https://ui-avatars.com/api/?name=' . urlencode($post->user->name ?? 'U') . '&background=6366f1&color=fff' }}" class="w-12 h-12 rounded-full">
            <div class="flex-1">
                ...
                <template x-if="editing">
                    <div class="space-y-3">
                        ...
                        <textarea x-model="editContent" rows="8" class="w-full px-4 py-2.5 border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 resize-none"></textarea>
                        <div class="flex space-x-2">
                            <button @click="editing = false" class="px-4 py-2 text-sm text-gray-600 hover:text-gray-800 transition">{{ __('Cancel') }}</button>
    ...
    <div class="mt-8">
        ...
        <div class="bg-white rounded-2xl border border-gray-100 p-6 mb-4">
            <div class="flex items-start space-x-3">
                <img src="{{ $comment->user->avatar ?? 'https://ui-avatars.com/api/?name=' . urlencode($comment->user->name ?? 'U') . '&background=6366f1&color=fff' }}" class="w-8 h-8 rounded-full">
                <div class="flex-1">
