...
<div class="bg-gradient-to-r from-indigo-500 to-purple-600 py-12">
    <div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <h1 class="text-2xl font-bold text-white">{{ __('Create Contest') }}</h1>
    </div>
</div>
        ...
        <form action="{{ route('contests.store') }}" method="POST">
            ...
            <div class="space-y-4">
                ...
                <div>
                    ...
                    <textarea name="description" rows="3" class="w-full px-4 py-2 border border-gray-200 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-transparent">{{ old('description') }}</textarea>
                </div>
                ...
                <div class="grid grid-cols-3 gap-4">
                    ...
                    <div>
                        ...
                        <input type="number" name="time_limit" value="{{ old('time_limit', 120) }}" min="1"
                            class="w-full px-4 py-2 border border-gray-200 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-transparent">
                    </div>
                </div>
            ...
            <div class="flex justify-end space-x-3 mt-6">
                <a href="{{ route('contests.index') }}" class="px-4 py-2 text-gray-600 hover:text-gray-800 transition">{{ __('Cancel') }}</a>
                <button type="submit" class="px-6 py-2 bg-indigo-600 text-white font-medium rounded-lg hover:bg-indigo-700 transition">
                    <i class="fas fa-save mr-1"></i> {{ __('Create Contest') }}
                </button>
            </div>
        </form>
    </div>
