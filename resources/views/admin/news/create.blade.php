...
@section('title', __('Create News') . ' - CodeMaster')
@section('header-title', __('Create News'))
@section('header-subtitle', __('Publish a new article or announcement'))
...
<div class="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
    <form action="{{ route('admin.news.store') }}" method="POST">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            <div class="lg:col-span-2 space-y-5">
                <div>
                    ...
                    <input type="text" name="title" value="{{ old('title') }}" required
                           class="admin-input w-full" placeholder="{{ __('Article title...') }}">
                    @error('title')<p class="text-red-500 text-xs mt-1">{{ $message }}</p>@enderror
                </div>

                <div>
                    <label class="admin-label">{{ __('Excerpt') }} <span class="text-gray-400 font-normal">(optional)</span></label>
                    <textarea name="excerpt" rows="2" class="admin-input w-full" placeholder="{{ __('Short summary (auto-generated if empty)...') }}">{{ old('excerpt') }}</textarea>
                </div>

                <div>
                    <label class="admin-label">{{ __('Content') }}</label>
                    <textarea id="news-content-tinymce" style="width:100%;min-height:400px">{{ old('content') }}</textarea>
                    <input type="hidden" name="content" id="news-content-hidden" value="{{ old('content') }}">
            ...
            <div class="space-y-5">
                <div>
                    <label class="admin-label">{{ __('Image URL') }} <span class="text-gray-400 font-normal">(optional)</span></label>
                    <input type="text" name="image" value="{{ old('image') }}" class="admin-input w-full"
                           placeholder="https://...">
                    @if(old('image'))
                    <img src="{{ old('image') }}" class="mt-2 rounded-lg w-full h-32 object-cover">
                    @endif
                </div>
                ...
                <div class="flex items-center gap-3 pt-4">
                    <button type="submit" class="px-5 py-2.5 bg-indigo-600 text-white text-sm font-medium rounded-xl hover:bg-indigo-700 transition">
                        <i class="fas fa-save mr-1"></i>{{ __('Publish') }}
                    </button>
...
<script>
(function() {
    let tags = {!! json_encode(old('tags') ? json_decode(old('tags')) : []) !!};
