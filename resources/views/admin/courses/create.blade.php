                ...
                <div class="grid grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <select name="level" id="level" class="w-full px-4 py-2.5 bg-gray-50 border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 @error('level') border-red-500 @enderror" required>
                            ...
                            <option value="Начальный" {{ old('level') === 'Начальный' ? 'selected' : '' }}>{{ __('Beginner') }}</option>
                            <option value="Средний" {{ old('level') === 'Средний' ? 'selected' : '' }}>{{ __('Intermediate') }}</option>
                            <option value="Продвинутый" {{ old('level') === 'Продвинутый' ? 'selected' : '' }}>{{ __('Advanced') }}</option>
                        </select>
                        @error('level')
                            <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                        @enderror
